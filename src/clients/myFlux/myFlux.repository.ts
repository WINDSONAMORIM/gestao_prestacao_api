...
interface DownloaderProcessResponse{ 
...
export class MyFluxService {
  ...
  async getTitle(
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/processos/${processId.id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  async downloaderProcess(
    data: number,
    token: string,
  ): Promise<DownloaderProcessResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/processos/${data}/empacotar`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const result : DownloaderProcessResponse = await response.json();
      console.log(`result downloaderProcess: ${typeof result}`);
