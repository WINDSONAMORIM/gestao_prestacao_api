...
import { ParamsAnualGrupo } from "../../../schemas/paramsShema.js";
import { ResponseAnaliseOrcamentariaAnualPorSubGrupo } from "../../../schemas/responseSchema.js";

export const resumoAnualPorSubgrupo = async (params: ParamsAnualGrupo):  Promise<ResponseAnaliseOrcamentariaAnualPorSubGrupo[]> => {
  const response = await prisma.$queryRaw<ResponseAnaliseOrcamentariaAnualPorSubGrupo[]>`
    WITH orcado AS (
      ...
      WHERE dt.ano = ${params.ano} AND sg.id_grupo = ${params.id_grupo}::varchar
